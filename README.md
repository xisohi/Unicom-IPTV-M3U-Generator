...
| 位置 | 原内容 | 修改后 |
|------|--------|--------|
| 开头 | 无 | 增加"基于 XX 二次开发"说明 |
| 功能列表 | 无桌面版 | 新增 🖥️ Windows 桌面版 GUI 工具 |
| 使用方法 | 只有 Web/Docker | 增加 **"快速开始（Windows 桌面版）"** 放在最前面 |
| 项目结构 | 无 `gui_launcher.py` | 新增 `gui_launcher.py` 和 `dist/` |

---

## 如果你不想大改，只改这几处也行

1. **开头增加一句话**：说明这是二次开发，有桌面版
2. **功能列表加一条**：Windows 桌面版 GUI 工具
3. **快速开始加一节**：桌面版的使用方法（下载 exe → 填信息 → 获取列表）
